# Feature: Phase 1 — Autograd Engine (ferrotorch-core)
...
- REQ-13: `Tensor<T>` must implement `std::ops` traits (`Add`, `Sub`, `Mul`, `Div`, `Neg`) for all four ownership combinations (`&Tensor op &Tensor`, `Tensor op Tensor`, `Tensor op &Tensor`, `&Tensor op Tensor`). Scalar-tensor operations must also be supported (`&Tensor * scalar`, `scalar * &Tensor`) where the scalar is `T`. `Index<usize>` and `Index<&[usize]>` must be implemented for single-element access. All operator overloads must build the computation graph identically to the corresponding `grad_fns` functions — they are syntax sugar, not a separate code path. This is essential for ergonomic parity with PyTorch's `c = a + b` syntax.
- REQ-14: `Tensor<T>` must implement `std::fmt::Display` with PyTorch-style formatting: tensor data (nested brackets, 4 decimal places by default), shape, dtype, and `grad_fn` name when present. Example: `tensor([[1.0000, 2.0000], [3.0000, 4.0000]], shape=[2, 2], dtype=f32, grad_fn=AddBackward)`. A `.print()` convenience method must print the Display output to stdout and return `&Self` for chaining. `Debug` must also be implemented with a more compact representation suitable for logging.
- REQ-15: All differentiable operations must be available as methods on `Tensor<T>`: `a.matmul(&b)`, `a.relu()`, `a.sigmoid()`, `a.sum()`, `a.mean()`, `a.reshape(&[2, 3])`, `a.transpose(0, 1)`, `a.exp()`, `a.log()`, `a.pow(2.0)`, `a.abs()`, `a.sqrt()`, `a.flatten(start_dim, end_dim)`, etc. The module-level functional API (`grad_fns::arithmetic::add(&a, &b)`) must also exist, but methods are the primary ergonomic interface. Methods delegate to the functional API internally — no duplicated backward logic.
- REQ-16: In-place operation variants must be provided: `add_(&other)`, `sub_(&other)`, `mul_(&other)`, `div_(&other)`, `zero_()`, `fill_(scalar)`, `clamp_(min, max)`. In-place ops modify the tensor's data buffer directly and return `Result<&mut Self, FerrotorchError>` for chaining. In-place ops are NOT tracked by autograd. Calling an in-place op on a leaf tensor with `requires_grad = true` must return `Err(FerrotorchError::InPlaceOnLeafWithGrad)`. Calling an in-place op on a non-leaf tensor that is part of a computation graph must return `Err(FerrotorchError::InPlaceOnGraphTensor)`. This matches PyTorch's semantics where in-place modification of tensors involved in autograd is forbidden.
- REQ-17: `Tensor<T>` must provide `.contiguous()` (returns self unchanged if already contiguous in memory, otherwise allocates a new contiguous tensor with the same data), `.clone()` (deep copy with independent storage and a new autograd identity — the clone is a graph operation so gradients flow back through it), and `.detach()` (returns a tensor sharing the same storage but with `requires_grad = false` and no `grad_fn`). `.is_contiguous()` must return a `bool`.
...
- [ ] AC-11: `cargo test -p ferrotorch-core` passes with 0 failures. Minimum 300 tests covering all grad_fns, creation functions, operator overloads, method API, in-place ops, display formatting, broadcasting, error paths, and edge cases (empty tensors, scalar tensors, 0-dim, large rank).
...
- [ ] AC-13: Operator overloading works for all four ownership combinations. `let c = &a + &b` builds a computation graph, `c.backward()` produces correct gradients on `a` and `b`. Scalar ops work: `let scaled = &a * 2.0f32` produces a tensor with all elements doubled. `Index` returns the correct element: `tensor[[0, 1]]` returns the element at row 0, col 1.
- [ ] AC-14: `println!("{}", tensor)` prints PyTorch-style output with data, shape, dtype, and grad_fn. A tensor without grad_fn omits that field. `.print()` returns `&Self` so `tensor.print().shape()` compiles.
- [ ] AC-15: Method syntax works for all grad_fns operations: `a.matmul(&b)`, `a.relu()`, `a.sum()`, `a.reshape(&[2, 3])`, etc. Methods and functional API produce identical computation graphs and identical backward results.
- [ ] AC-16: `tensor.add_(&other)` modifies data in place. Calling `leaf_tensor.add_(&other)` where `leaf_tensor.requires_grad() == true` returns `Err(InPlaceOnLeafWithGrad)`. Calling `non_leaf.mul_(&other)` where `non_leaf` has a `grad_fn` returns `Err(InPlaceOnGraphTensor)`. `tensor.zero_()` sets all elements to zero.
- [ ] AC-17: `.contiguous()` on a transposed (non-contiguous) tensor returns a new contiguous tensor with correct data. `.clone()` returns a deep copy that participates in autograd: if `b = a.clone()` and `b.backward()`, then `a.grad` is correct. `.detach()` returns a view with no grad_fn and `requires_grad = false`.
- [ ] AC-18: Arithmetic operations with broadcasting work correctly: adding a `[3, 1]` tensor to a `[1, 4]` tensor produces a `[3, 4]` result. Backward through broadcasted ops correctly reduces gradients along the broadcasted dimensions (sum-reducing the gradient to match each input's original shape).
- [ ] AC-19: Tensor creation, elementwise ops, and matmul must not have more than 2x wall-clock overhead compared to equivalent direct ferray operations. Measured via a benchmark that performs the same computation through ferrotorch (with `requires_grad = false`) and through raw ferray, comparing elapsed time. This measures framework dispatch overhead, not algorithm speed.
...
│   ├── ops_trait.rs              # std::ops trait impls (Add, Sub, Mul, Div, Neg, Index)
│   ├── display.rs                # Display, Debug formatting, .print()
│   ├── inplace.rs                # In-place ops: add_, sub_, mul_, div_, zero_, fill_, clamp_
│   ├── autograd/
│   │   ├── mod.rs
...
└── tests/
    ...
    ├── test_ops_trait.rs         # Operator overloading: +, -, *, /, neg, index, scalar ops
    ├── test_display.rs           # Display formatting, .print()
    ├── test_inplace.rs           # In-place ops, error on leaf with grad, error on graph tensor
    ├── test_broadcast.rs         # Broadcasting correctness in forward and backward
    └── test_overhead.rs          # Framework overhead benchmark vs raw ferray
```
...
### Operator Overloading and Method API

`Tensor<T>` implements `std::ops` traits via a macro that generates all four ownership combinations for binary ops:

```rust
// Macro generates: &T op &T, T op T, T op &T, &T op T
// Each delegates to grad_fns::arithmetic::<op>()
impl<T: Element> Add for &Tensor<T> {
    type Output = Result<Tensor<T>, FerrotorchError>;
    fn add(self, rhs: &Tensor<T>) -> Self::Output {
        grad_fns::arithmetic::add(self, rhs)
    }
}
```

Note: `Output` is `Result<Tensor<T>, FerrotorchError>` since operations can fail (shape mismatch, device mismatch). Users handle this with `?` in fallible contexts or `.unwrap()` in scripts. This differs from PyTorch which panics on invalid ops — ferrotorch follows Rust conventions.

Scalar-tensor ops are implemented for all `T: Element` types:
```rust
impl<T: Element> Mul<T> for &Tensor<T> { ... }  // tensor * scalar
impl<T: Element> Mul<&Tensor<T>> for f32 { ... } // scalar * tensor (f32 only; also f64, bf16)
```

**Method API** is implemented in `ops/elementwise.rs` and `ops/linalg.rs` as `impl<T: Element> Tensor<T>` blocks. Each method delegates to the corresponding `grad_fns` function:

```rust
impl<T: Element> Tensor<T> {
    pub fn matmul(&self, other: &Tensor<T>) -> Result<Tensor<T>, FerrotorchError> {
        grad_fns::linalg::matmul(self, other)
    }
    pub fn relu(&self) -> Result<Tensor<T>, FerrotorchError> {
        grad_fns::activation::relu(self)
    }
    pub fn sum(&self) -> Result<Tensor<T>, FerrotorchError> {
        grad_fns::reduction::sum(self, None, false)
    }
    // ... etc for all grad_fns operations
}
```

**In-place operations** are implemented directly on `Tensor<T>` and mutate the storage buffer. They do not create graph nodes:

```rust
impl<T: Element> Tensor<T> {
    pub fn add_(&mut self, other: &Tensor<T>) -> Result<&mut Self, FerrotorchError> {
        if self.requires_grad && self.is_leaf {
            return Err(FerrotorchError::InPlaceOnLeafWithGrad);
        }
        if self.grad_fn.is_some() {
            return Err(FerrotorchError::InPlaceOnGraphTensor);
        }
        // ... modify storage in place ...
        Ok(self)
    }
    pub fn zero_(&mut self) -> Result<&mut Self, FerrotorchError> { ... }
    pub fn fill_(&mut self, value: T) -> Result<&mut Self, FerrotorchError> { ... }
}
```

### Display and Formatting

`Display` renders tensors in PyTorch style with nested brackets and 4-decimal precision:

```
tensor([[1.0000, 2.0000],
        [3.0000, 4.0000]], shape=[2, 2], dtype=f32, grad_fn=AddBackward)
```

For large tensors (> 6 elements along any dimension), values are summarized with `...` like PyTorch. The `grad_fn` field is omitted for leaf tensors. The `.print()` method calls `println!("{}", self)` and returns `&Self`.
...
|------|-----------|-------------|
| `arithmetic.rs` | add, sub, mul, div, neg, pow, sqrt, abs | Elementwise with NumPy-style broadcasting: inputs are broadcast to a common shape before the forward op. Backward must reduce (sum) gradients along any dimensions that were broadcast-expanded to restore each input's original shape. add/sub: grad passes through (with possible reduction); mul: `grad * other` (with reduction); div: `grad / other` and `-grad * a / b^2` (with reduction); neg: `-grad`; pow: `grad * b * a^(b-1)` and `grad * a^b * ln(a)`; sqrt: `grad / (2 * sqrt(a))`; abs: `grad * sign(a)` |
...
| `shape.rs` | reshape, transpose, permute, expand, contiguous, clone, cat, stack, split, squeeze, unsqueeze, flatten | Inverse shape ops: reshape grad back, transpose grad, split grad at cat boundaries; clone: identity VJP (grad passes through unchanged); contiguous: identity VJP |
...
pub enum FerrotorchError {
    ...
    #[error("in-place operation on leaf tensor that requires grad is forbidden")]
    InPlaceOnLeafWithGrad,
    #[error("in-place operation on a tensor that is part of the computation graph is forbidden")]
    InPlaceOnGraphTensor,
    #[error(transparent)]
...
1. **Numerical gradient checks**: For every `GradFn`, compare analytic gradient against finite-difference approximation: `(f(x+h) - f(x-h)) / 2h`. Use `h = 1e-5` for f32, `h = 1e-8` for f64. Must include broadcasted inputs (e.g., `[3, 1] + [1, 4]`) to verify gradient reduction.
...
3. **Edge cases**: Empty tensors, scalar (0-dim) tensors, tensors with size-0 dimensions, very high rank (8+), contiguous vs non-contiguous inputs, broadcasted operands of different ranks.
...
6. **Operator overloading**: All four ownership combos for each binary op, scalar-tensor both directions, `Index` trait for element access. Verify operator results match functional API results exactly (bitwise identical).
7. **In-place ops**: Verify mutation occurs, verify error on leaf-with-grad, verify error on graph tensor, verify chaining (`tensor.zero_().fill_(1.0)`).
8. **Display formatting**: Verify output matches PyTorch format for 1D, 2D, 3D, scalar, and large (summarized) tensors. Verify `grad_fn` appears only when present.
9. **Framework overhead**: Benchmark `Tensor<f32>` elementwise add, matmul, and creation against raw ferray equivalents. Assert < 2x overhead for `requires_grad = false` path.
