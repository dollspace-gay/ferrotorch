# Feature: Phase 0 — Build Orchestrator Agent
...
- REQ-4: The orchestrator must prevent out-of-order work — a phase must not be dispatched until all phases in its `depends_on` list have all subissues closed, `cargo test -p <crate>` passing, AND `cargo clippy -p <crate> -- -D warnings` clean. External prerequisites must be manually marked resolved by the human.
...
- REQ-8: The orchestrator must write a `.design/build-status.json` file after each invocation, persisting the current state of all phases (status, subissue counts, active agents, blockers, phase completion timestamps). This file is the queryable snapshot of build progress — reading it must not require re-scanning all issues or tmux sessions.
- REQ-9: On startup, the orchestrator must validate the dependency graph: (a) all `design_doc` paths referenced in `.design/dependency-graph.json` exist on disk, (b) all phase `id` values are unique, (c) no circular dependencies exist in `depends_on` chains, and (d) every `depends_on` entry references a valid phase `id`. Validation failures abort the run with a clear diagnostic.
- REQ-10: The orchestrator must monitor agent health via a heartbeat check — if a tmux session has produced no new output for 10 minutes, flag it as potentially stuck, log a warning to the status dashboard, and create a `needs-human` escalation issue.
- REQ-11: Phase completion requires all three gates: (a) all subissues closed, (b) `cargo test -p <crate>` passing, and (c) `cargo clippy -p <crate> -- -D warnings` clean. If clippy or tests fail after all subissues are closed, the phase remains in `FAILING` status and the orchestrator escalates.
- REQ-12: The orchestrator must log approximate token usage per agent session for budget visibility. After each agent session completes (tmux session exits), the orchestrator records the session's token estimate (from `crosslink` metadata or tmux scrollback size heuristic) in `.design/build-status.json` under a `cost_tracking` key.
- REQ-13: `/orchestrate` must be idempotent and resumable. Running it repeatedly must pick up where it left off — it reads `.design/build-status.json` and crosslink issue state to determine what is already complete, in progress, or failed, and only dispatches work for phases/subissues not yet started. Completed work is never re-dispatched.
...
- [ ] AC-2: Running `/orchestrate` when Phase 1 is incomplete (open subissues exist) does not dispatch Phase 2 work. Running it when Phase 1 is complete (all subissues closed, `cargo test -p ferrotorch-core` passes, `cargo clippy -p ferrotorch-core -- -D warnings` clean) dispatches Phase 2 if its design doc exists.
...
- [ ] AC-8: After each `/orchestrate` invocation, `.design/build-status.json` exists and contains: phase statuses (`waiting`, `in_progress`, `failing`, `complete`), subissue counts per phase, active agent sessions, blocker summaries, phase completion timestamps (ISO 8601), and a `last_updated` field. The file is valid JSON parseable by `jq`.
- [ ] AC-9: Running `/orchestrate` against a dependency graph with a circular dependency (e.g., phase 2 depends on phase 3, phase 3 depends on phase 2) aborts with an error message naming the cycle. Running it with a `depends_on` referencing a nonexistent phase ID aborts with an error naming the invalid reference. Running it with duplicate phase IDs aborts naming the duplicates.
- [ ] AC-10: If a tmux agent session produces no new output for 10 minutes (measured by scrollback buffer timestamp or polling), the status dashboard shows it as `STUCK` and a `needs-human` issue is created referencing the stuck session and phase.
- [ ] AC-11: Phase completion requires `cargo clippy -p <crate> -- -D warnings` returning exit code 0 in addition to `cargo test -p <crate>`. If clippy fails after all subissues are closed, the phase shows as `FAILING` (not `COMPLETE`) and the orchestrator escalates with the clippy output.
- [ ] AC-12: `.design/build-status.json` contains a `cost_tracking` object with per-agent-session entries recording: session name, phase, approximate token count, and session duration. The orchestrator updates this after each agent session completes.
- [ ] AC-13: Running `/orchestrate` twice in succession (without any external changes) produces no new dispatches, no duplicate issues, and no errors. The second run reads persisted state and determines all in-progress work is already dispatched.
...
```yaml
---
allowed-tools: Bash(crosslink *), Bash(tmux *), Bash(cargo test *), Bash(cargo clippy *), Bash(cat *), Bash(ls *), Bash(git *), Bash(date *), Bash(jq *)
...
- Build status: !`cat .design/build-status.json 2>/dev/null || echo "no status file — first run"`
```
...
Before any dispatch, the orchestrator validates the dependency graph:

1. Parse `.design/dependency-graph.json` — abort with diagnostic if malformed
2. Check all phase `id` values are unique — abort naming duplicates if not
3. Check all `depends_on` entries reference valid phase `id` values — abort naming invalid references if not
4. Check for circular dependencies via topological sort — abort naming the cycle if one exists
5. Check all `design_doc` paths exist on disk — warn (do not abort) for missing docs, since those phases simply cannot be dispatched yet

If validation passes, load `.design/build-status.json` (if it exists) to restore persisted state from previous runs. If the status file does not exist, initialize all phases as `waiting`.

### Dispatch Flow

1. Orchestrator reads `.design/dependency-graph.json` and `.design/build-status.json`
2. Skip any phase already marked `complete` in persisted state (resume-after-failure)
3. Skip any phase already marked `in_progress` whose agents are still running (idempotent re-entry)
4. For each remaining phase whose `depends_on` phases are all complete:
...
5. Monitor via `crosslink issue list` and `tmux list-sessions`:
      - When all subissues for a phase are closed, run `cargo test -p <crate>` and `cargo clippy -p <crate> -- -D warnings`
      - If both pass, mark the phase `complete` in `build-status.json` with a completion timestamp
      - If either fails, mark the phase `failing` and escalate with the failure output
      - Check agent heartbeats: if any tmux session has no new output for 10 minutes, flag as `STUCK` and escalate
6. Record approximate token usage for any completed agent sessions in `build-status.json` under `cost_tracking`
7. Write updated state to `.design/build-status.json` (phase statuses, subissue counts, agent sessions, cost tracking, timestamps)
...
- A sub-agent's tmux session has produced no new output for 10 minutes (heartbeat timeout — detected by comparing scrollback buffer snapshots or polling `tmux capture-pane` at intervals)
...
- `cargo clippy -p <crate> -- -D warnings` fails after all subissues in a phase are closed
- Dependency graph validation fails on startup (circular deps, invalid references, duplicate IDs)
...
```
ferrotorch build status (last updated: 2026-03-14T18:32:00Z):
  ...
  Phase 1: Autograd Engine     [IN PROGRESS] 7/13 subissues closed
    Step 5 (grad_fns):  5 agents running in parallel
      ...
      feat-gradfn-shape        [tmux]  STUCK     No output for 12 minutes
  ...
  Blockers requiring human attention:
    #45: softmax numerical instability — grad_fns/activation.rs (Phase 1, step 5)
    #46: Agent STUCK — feat-gradfn-shape, no output for 12 min (Phase 1, step 5)

  Cost tracking:
    Completed sessions: 6 agents, approx 1.7M tokens total
    Active sessions:    5 agents running
```

### Progress Persistence

After each invocation, the orchestrator writes `.design/build-status.json`:

```json
{
  "last_updated": "2026-03-14T18:32:00Z",
  "phases": {
    "1": {
      "status": "complete",
      "subissues_total": 13,
    ...
    "2": {
      ...
      "completed_at": null,
      "active_agents": ["feat-nn-linear", "feat-nn-conv2d"]
    },
    "3": {
      "status": "waiting",
      "subissues_total": 0,
  ...
  "blockers": [
    {
      "issue_id": 45,
      "phase": 2,
      "summary": "Conv2d padding mode ambiguity",
  ...
  "cost_tracking": {
    "sessions": [
      {
        "name": "feat-gradfn-arithmetic",
        "phase": 1,
        "approximate_tokens": 285000,
        "duration_minutes": 47,
        "completed_at": "2026-03-12T11:30:00Z"
      }
    ],
    "total_approximate_tokens": 3200000
  }
}
```

This file enables querying build status without re-scanning issues or tmux sessions. The orchestrator reads it on startup for resume-after-failure semantics.

### File Locations
- Dependency graph: `.design/dependency-graph.json`
- Build status (persisted): `.design/build-status.json`
...
- Cost optimization or budget enforcement — the orchestrator tracks token usage for visibility but does not enforce spending limits or throttle agents
