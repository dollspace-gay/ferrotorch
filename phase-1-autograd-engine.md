...
- REQ-6: VJP implementations must exist for all operations listed in the grad_fns table below (arithmetic, reduction, linalg, activation, shape, indexing, comparison categories — 40+ operations).
...
- REQ-12: Tensor operations must delegate to ferray-ufunc for CPU elementwise math (exp, log, sin, cos, etc.) and ferray-linalg for linear algebra where the element type is f64. For f32 tensors, the crate must provide its own matmul implementation using faer directly, since ferray-linalg is f64-only.
...
- [ ] AC-10: `Tensor<f32>` matmul produces correct results using the faer f32 backend (not f64 promotion). Verified against PyTorch `torch.mm()` output.
...
│   │   ├── norm.rs               # layer_norm, batch_norm, group_norm (functional versions)
...
│       ├── linalg.rs             # matmul dispatch: f64 → ferray-linalg, f32 → faer direct
...
pub struct Tensor<T: Element = f32> {
    ...
    grad: RefCell<Option<Box<Tensor<T>>>>,  // Interior mutability for backward
...
`Arc<TensorStorage<T>>` enables multiple graph nodes to reference the same data without cloning. `RefCell` on `grad` allows backward to write gradients without `&mut self`.
...
**f32 matmul path**: Since ferray-linalg is f64-only, `ops/linalg.rs` dispatches matmul based on dtype:
- `f64`: delegate to `ferray_linalg::matmul()`
- `f32`: call `faer::Mat::<f32>` directly, performing the same matmul logic but in f32
- `bf16`: promote to f32, compute, demote (until native bf16 SIMD is available)

### Grad Functions Table
...
| `norm.rs` | layer_norm, batch_norm, group_norm | Standard VJPs with running stats handling for batch_norm |
| `conv.rs` | conv1d, conv2d, conv3d, conv_transpose2d | im2col + matmul forward; col2im for backward |
| `pool.rs` | max_pool2d, avg_pool2d, adaptive_avg_pool2d | max: index-based routing; avg: uniform distribution |
| `dropout.rs` | dropout | Reapply the same mask during backward, scaled by `1/(1-p)` |
| `embedding.rs` | embedding | Sparse gradient: scatter grad rows to embedding indices |
| `loss.rs` | cross_entropy, mse, nll, bce, huber, ctc | Combined LogSoftmax+NLL for cross_entropy stability |
| `attention.rs` | scaled_dot_product_attention | VJP through softmax(QK^T/sqrt(d))V with flash-attention-aware memory |
...
| `ferray-linalg` | workspace | f64 matmul/decompositions |
| `ferray-random` | workspace | Random tensor creation (rand, randn) |
| `faer` | 0.24 | f32 matmul (direct, bypassing ferray-linalg) |
