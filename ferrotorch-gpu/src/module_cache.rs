...
/// Locate the CUDA toolkit include directory.
///
/// Check (in order):
...
/// - `/usr/local/cuda-12.8`
///
/// Returns `None` if none exist — callers that need nvrtc with CUDA
/// headers should error out visibly.
#[cfg(feature = "cuda")]
fn cuda_include_dir() -> Option<String> {
    ...
    .flatten()
    {
        let inc = format!("{candidate}/include");
        if std::path::Path::new(&format!("{inc}/cuda_bf16.h")).exists() {
            return Some(inc);
        }
    }
    None
...
/// Compilation error for [`get_or_compile_cuda`].
///
/// Wraps either the nvrtc compile error (with its log) or a driver
/// error from `load_module` / `load_function`.
#[cfg(feature = "cuda")]
#[derive(Debug)]
pub enum CudaCompileError {
    /// The CUDA C++ source failed to compile via nvrtc.
    Nvrtc(cudarc::nvrtc::CompileError),
    /// The driver rejected the compiled module or the requested
    /// function was not present in the module.
    Driver(DriverError),
    /// No CUDA include directory could be located.
    MissingIncludes,
}

#[cfg(feature = "cuda")]
impl std::fmt::Display for CudaCompileError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            CudaCompileError::Nvrtc(e) => write!(f, "nvrtc compile error: {e:?}"),
            CudaCompileError::Driver(e) => write!(f, "driver error: {e}"),
            CudaCompileError::MissingIncludes => write!(
                f,
                "no CUDA include directory found (set CUDA_HOME or install cuda-cudart-devel)"
            ),
        }
    }
}
...
/// Compile CUDA C++ source via nvrtc on first use, cache the resulting
/// function.
///
/// This is the preferred entry point for new kernels. Writing the math
/// in CUDA C++ — with helpers like `__nv_bfloat16`, `__bfloat162float`,
/// `__float2bfloat16` from `<cuda_bf16.h>` — lets nvcc handle dtype
/// conversion, register allocation, tensor-core instructions, and
/// target-specific codegen. This matches PyTorch's approach.
///
/// # Arguments
///
/// - `ctx`            — CUDA context (from `device.context()`).
/// - `cuda_src`       — CUDA C++ source (a `&'static str` constant). The
///   source should declare its entry points as `extern "C" __global__`.
/// - `kernel_name`    — entry-point name inside the source.
/// - `device_ordinal` — CUDA device ordinal.
///
/// The first successful compile is cached under `kernel_name`; later
/// calls return a cheap clone.
#[cfg(feature = "cuda")]
pub fn get_or_compile_cuda(
    ctx: &Arc<CudaContext>,
    cuda_src: &'static str,
    kernel_name: &'static str,
    device_ordinal: u32,
) -> Result<CudaFunction, CudaCompileError> {
    let key = (kernel_name, device_ordinal);
    {
    ...
    let include_dir = cuda_include_dir().ok_or(CudaCompileError::MissingIncludes)?;
    let opts = cudarc::nvrtc::CompileOptions {
        include_paths: vec![include_dir],
        ..Default::default()
    };
    let ptx = cudarc::nvrtc::compile_ptx_with_opts(cuda_src, opts)
        .map_err(CudaCompileError::Nvrtc)?;

    let module = ctx.load_module(ptx).map_err(CudaCompileError::Driver)?;
    let func = module
        .load_function(kernel_name)
        .map_err(CudaCompileError::Driver)?;
